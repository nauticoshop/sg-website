    ...
    <section className="py-24 lg:py-32 px-6 lg:px-12 bg-ink text-canvas">
      <div className="max-w-[1440px] mx-auto">
        <header className="text-center mb-16 lg:mb-20 max-w-4xl mx-auto">
          ...
          <h2 className="font-sans font-extrabold text-4xl md:text-5xl lg:text-6xl xl:text-7xl leading-[1.05] tracking-tight text-canvas text-balance">
            The numbers that separate us from everyone else pitching luxury.
          </h2>
        </header>

        <ul className="grid grid-cols-2 lg:grid-cols-4 divide-y divide-x divide-canvas/15 border border-canvas/15 lg:border-0">
          {stats.map((stat, i) => (
            <li
              key={i}
              className="px-6 py-12 lg:px-8 lg:py-16 text-center flex flex-col justify-center min-h-[220px]"
            >
              <p className="font-sans font-extrabold text-5xl md:text-6xl lg:text-7xl xl:text-8xl text-canvas leading-none mb-5 tracking-tight">
                <CountUp value={stat.value} />
              </p>
              <p className="text-sm lg:text-base text-canvas/65 leading-snug max-w-[240px] mx-auto">
                {stat.label}
              </p>
            </li>
          ))}
